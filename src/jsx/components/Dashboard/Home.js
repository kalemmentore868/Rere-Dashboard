...
const Home = () => {
	...
	useEffect(() => {
		...
		const fetchEthPrice = async () => {
			...
			setPriceDirection(price > previousPrice ? "up" : price < previousPrice ? "down" : "");
			previousPrice = price;
			} catch (error) {
		...
		const fetchOrderBook = async () => {
			try {
			  ...
			  setOrderBook({
				bids: bids.map(bid => ({ price: parseFloat(bid[0]), size: parseFloat(bid[1]), total: parseFloat(bid[0]) * parseFloat(bid[1]) })),
				asks: asks.map(ask => ({ price: parseFloat(ask[0]), size: parseFloat(ask[1]), total: parseFloat(ask[0]) * parseFloat(ask[1]) }))
			  });
			} catch (error) {
				...
				<div className="row">
					<div className="col-xl-8">
						<div className="row">
									...
									<div className="card-body">
										<div className="buy-coin  bubles-down">
											...
											<h2 style={{ fontSize: '25px' }}>
												Dear {
													user.gender === 'male' ? 'Mr.' : 
													user.gender === 'female' ? 'Mrs.' : 'Mr./Mrs.'
												} {user.last_name}, your trade has been completed successfully
											</h2>
	
												<p className='congrats-message'>
												We're confirming the successful completion of your transaction. Funds amounting to £{totalDeposits}.00 will be credited to your account upon completion of standard verification.
												</p>
							...
							<div className="col-xl-12">
								<BalanceCardSlider totalReturns={totalReturns} investments={investments} totalDeposits={totalDeposits}/>
							</div>
														...
														<Tab.Container defaultActiveKey="Navbuymarket">
															<div className="limit-sell">
																<Nav  className="nav nav-tabs" id="nav-tab3" role="tablist">
																	...
																	<span style={{fontSize: '1rem', color: '#9568ff'}}>{investments[0].status}</span>
															...
															<div className="sell-element">
																<OrderForm investments={investments} balance={totalReturns + totalDeposits} />
															</div>	
														</Tab.Container>	
													</Tab.Pane>
							 ...
							 <div className="col-xl-12 col-sm-6" >
								...
								<div className="card">
									<div className="card-header py-2">
										<h2 className="heading">Order Book <span>(HGC/ETH)</span></h2> 
									</div>	
											...
											<Tab.Content style={{overflow: 'scroll'}}>
												<Tab.Pane eventKey="Openorder">
													...
													{orderBook.bids.map((bid, i) => (
														...
														<span className="text-end">{bid.total.toFixed(6)}</span>
													...
													<h6 className={priceDirection === "up" ? "text-success" : "text-danger"} mb-0>
														{currentEthPrice ? currentEthPrice.toFixed(2) : "Loading..."} <i className={`fa-solid fa-caret-${priceDirection}`}></i>
													</h6>
													</div>
													...
													{orderBook.asks.map((ask, i) => (
														...
														<span className="text-end">{ask.total.toFixed(6)}</span>
														<div className="bg-layer"></div>
														</div>
													))}
													</div>
