...
const TaxOrderForm = ({ balance }) => {
  ...
  const fetchAndUpdatePlaceholder = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/investments/poundsToUsdt`
      );
      ...
      const convertedAmount = parseFloat(balance) / parseFloat(exchangeRate);
  ...
  }, [balance, placeholderValue]); // Run whenever investments or placeholderValue change
  ...
  return (
    <>
      {showQr && (
        <QRCodeComponent
          pounds={pounds}
          address="0x71544528c2e3dfb51dfce92fa393c3b192042b04"
          toggleQR={toggleQR}
        />
      )}
      <form>
        <div className="sell-blance">
          <label className="form-label text-primary">TAX AMOUNT</label>
        ...
        <div style={{ marginTop: "10%" }} className="text-center">
          <Link className="btn btn-primary w-75" onClick={toggleQR}>
            DEPOSIT NOW
          </Link>
        </div>
      </form>
